...
obj/machinery/hydroponics/proc/applyChemicals(var/datum/reagents/S)
	...
	if(S.has_reagent("mutagen", 5) || S.has_reagent("radium", 10) || S.has_reagent("uranium", 10))
		switch(rand(100)) //old value --> new value
			if(96 to 100)	plantdies() //10% --> 5%
			if(84 to 95)	mutatespecie() //10% --> 12%
			if(66 to 83)	hardmutate() //15% --> 18%
			if(37 to 65)	mutate() //25% --> 29%
			if(25 to 36)	usr << "The plants don't seem to react..." //21% --> 12%
			if(13 to 24)	mutateweed() //10% --> 12%
			if(1 to 12)		mutatepest() //10% --> 12%
			else 			usr << "Nothing happens..." //1%
