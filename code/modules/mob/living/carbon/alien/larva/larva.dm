			...
			if (prob(90))
				...
				if ((stat != DEAD) && (damage > 4.9))
					...
					for(var/mob/O in viewers(M, null))
						if ((O.client && !( O.blinded )))
							O.show_message(text("<span class='userdanger'>[] has weakened []!</span>", M, src), 1, "<span class='danger'>You hear someone fall.</span>", 2)
				adjustBruteLoss(damage)
				updatehealth()
			else
	...
	switch(M.a_intent)
		...
		if ("help")
			...
			if (health > 0)
				...
				M << "<span class='green'><B>[name] is too injured for that.</B></span>"
	return
